...
DELAYS="MAX";
NOTIMINGCHECKS="";
while getopts m:gs:dtny: option
  do case "${option}" in
    m) MODE=${OPTARG};;
...
case "$MODE" in
  ...
  "FPGA") KMIE_IMPLEMENT="-define KMIE_IMPLEMENT_FPGA" ;
    echo "ERROR: FPGA mode not available for post-layout simulations.";
    exit -1
    ;;
...
DELAY_PARAMS=""
case "$DELAYS" in
  "MIN") DELAY_PARAMS="-mindelays" ;;
  "MAX") DELAY_PARAMS="-maxdelays" ;;
  ?) echo "Bad delay parameter: $DELAYS"; exit -1 ;;
esac
#------------------------------------------------------------------------------
...
if [[ "$MODE" == "ASIC" ]]; then
  ...
  TSMC_PARAMS+=" -v /cad/dk/PDK_CRN45GS_DGO_11_25/digital/Front_End/verilog/tcbn40lpbwp_200a/tcbn40lpbwp.v"
  TSMC_PARAMS+=" -xmhierarchy \"tb_mtm_riscv_soc.u_mtm_riscv_soc_top\""
  TSMC_PARAMS+=" +nowarnTRNNOP"
  TSMC_PARAMS+=" -define NTC"
  TSMC_PARAMS+=" -define RECREM"
  case "$DELAYS" in
    "MIN") TSMC_PARAMS+=" -v /cad/dk/PDK_CRN45GS_DGO_11_25/IMEC_RAM_generator/ram/ts1n40lpb4096x32m4m_250a/VERILOG/ts1n40lpb4096x32m4m_250a_ff1p21v0c.v" ;;
    "MAX") TSMC_PARAMS+=" -v /cad/dk/PDK_CRN45GS_DGO_11_25/IMEC_RAM_generator/ram/ts1n40lpb4096x32m4m_250a/VERILOG/ts1n40lpb4096x32m4m_250a_ss0p99v125c.v" ;;
  esac
fi
#------------------------------------------------------------------------------
TCFDUMP_PARAMS=""
if [[ "$TCFDUMP" == "YES" ]]; then
  echo "TCF dump enabled."
  TCFDUMP_PARAMS+=" -tcl -input scripts/dumptcf.tcl"
  TCFDUMP_PARAMS+=" -access r"
fi
#------------------------------------------------------------------------------
TIMINGCHECK_PARAMS=""
if [[ "$NOTIMINGCHECKS" == "YES" ]]; then
  echo "Timing checks disabled."
  TIMINGCHECK_PARAMS+=" -notimingchecks"
fi
#------------------------------------------------------------------------------
# prepare post-layout data
...
if [[ "$NOTIMINGCHECKS" == "YES" ]]; then
  echo "Timing checks were DISABLED during simulation." | tee -a $LOG_FILE
fi
if [[ "$TCFDUMP" == "YES" ]]; then
  echo "TCF dump was ENABLED during simulation" | tee -a $LOG_FILE
fi
